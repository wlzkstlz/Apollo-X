...
void SetSupplyState(uint32_t state)
{
	uint32_t supply_state=state;
	STMFLASH_Write(FLASH_PILOT_STATE_ADDR,&supply_state,1);
}
uint32_t GetSupplyState(void)
{
	uint32_t supply_state=0;
	STMFLASH_Read(FLASH_PILOT_STATE_ADDR,&supply_state,1);
	return supply_state;
}
